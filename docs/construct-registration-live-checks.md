...
| --- | --- | --- | --- |
| ◐ | **Before merging a substantial registration backend branch** | Run the focused registration/model-candidate tests **and the full NASolve regression suite**. | **Focused bundle: 44/44 green on Birch. Full suite still pending.** Merge only after the full suite is green and no unrelated existing tests regress. |
...
The full NASolve regression suite remains the only outstanding pre-merge gate,
tracked in [`construct-registration-intent.json`](construct-registration-intent.json).
