...
| --- | --- | --- | --- |
| ☐ | **Before merging a substantial registration backend branch** | Run the focused registration/model-candidate tests **and the full NASolve regression suite**. | Focused tests and full suite are green; no unrelated existing tests regress. |
...
Newer Birch work is intentionally tracked as pending validation in
[`construct-registration-intent.json`](construct-registration-intent.json)
until the next local focused/full regression run.
